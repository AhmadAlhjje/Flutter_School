...
- **بدون إنترنت يفتح التطبيق كما كان**: آخر ما جاء من الخادم لكل شاشة (الرئيسية، المواد، المدرسين،
  الدروس، الجلسات، الإشعارات، حسابي) محفوظ على الهاتف ويُعرض فوراً مع سطر صغير «أنت غير متصل»، بدون
  جلب أي جديد. يحاول التطبيق الاتصال كل 15 ثانية، وعند عودة الإنترنت تتحدّث الشاشات وتظهر رسالة
  خضراء. الحفظ في مساحة التطبيق الخاصة ويُمسح عند تسجيل الخروج (`lib/core/network/offline_cache.dart`).
- **الملفات تُحمَّل مرة واحدة** مثل الفيديوهات: بعد أول فتح تُحفظ على الهاتف (تظهر «على الجهاز»)
  وتفتح منه بعد ذلك، حتى بدون إنترنت. مع الإنترنت يُتحقق من الصلاحية فقط؛ ملف أُغلق عن الطالب يُحذف من
  هاتفه.
- **الإشعارات تُقرأ بمجرد فتح صفحتها** (لا حاجة للضغط عليها)؛ الجديدة تبقى مميّزة خلال تلك الزيارة.
