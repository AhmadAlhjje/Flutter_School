...
- **بدون إنترنت يفتح التطبيق كما كان**: آخر ما جاء من الخادم لكل شاشة (الرئيسية، المواد، المدرسين،
  الدروس، الجلسات، الإشعارات، حسابي) محفوظ على الهاتف ويُعرض فوراً كما هو (بدون أي رسالة)، بدون جلب
  أي جديد. يحاول التطبيق الاتصال كل 15 ثانية، وعند عودة الإنترنت تتحدّث الشاشات وتظهر رسالة
  خضراء. الحفظ في مساحة التطبيق الخاصة ويُمسح عند تسجيل الخروج (`lib/core/network/offline_cache.dart`).
- **الملفات تُحمَّل مرة واحدة** مثل الفيديوهات: بعد أول فتح تُحفظ على الهاتف (تظهر «محمّل» بالأخضر)
  وتفتح منه بعد ذلك، حتى بدون إنترنت. مع الإنترنت يُتحقق من الصلاحية فقط؛ ملف أُغلق عن الطالب يُحذف من
  هاتفه.
- **الإشعارات رسائل فقط**: تُقرأ بمجرد فتح صفحتها، والضغط عليها لا ينقل لأي صفحة؛ الجديدة تبقى مميّزة
  خلال تلك الزيارة.
- **الرسائل الناجحة خضراء والفاشلة حمراء** (مثل «تم تغيير كلمة المرور»، انتهاء تحميل فيديو أو فشله).
